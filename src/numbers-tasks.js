...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const absAVector = Math.sqrt(x1 ** 2 + y1 ** 2);
  const absBVector = Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(scalarProizvedenie / (absAVector * absBVector));
}
